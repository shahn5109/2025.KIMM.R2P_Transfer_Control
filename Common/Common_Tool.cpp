...
CString ReturnSelect(UINT ret)
{
	switch(ret) {
		...
		default: return _T("NOT-DEFINE");
	}
	return _T("NOT-DEFINE");
}

/// 현재 우린 이걸 많이 쓰고 있다.
/// 에러 메세지를 띄울땐 부저와 적색등을 키고, 부저오프 버튼을 만들겠다.
//  확인으로 다이얼로그를 닫을때 부저와 적색등을 끄겠다.
UINT MyMessageBox(TCHAR* message, TCHAR* title, UINT type, TCHAR* YesBut/*=_T("예")*/, TCHAR* NoBut/*=_T("아니오")*/)
{
	...
	_tcscpy_s(dlg.m_message, message);
	_tcscpy_s(dlg.m_title, title);
	
	_tcscpy_s(dlg.m_strYes, YesBut);
	_tcscpy_s(dlg.m_strNo, NoBut);

	if( type & M_ICONERROR )
	{
		/// 에러 메세지를 띄울땐 부저와 적색등을 키고, 부저오프 버튼을 만들겠다. ( Abort 버튼을 부저정지버튼으로 하였다.)
		//  다이얼로그를 닫을때 부저와 적색등을 끄겠다.
		pSystem.GetIO()->SetBuzzer( 0 ) ;
	}

	UINT ret= dlg.DoModal();

	CString strText;  
	strText.Format(_T("[MessageBox] [Message=>%s] [Title=>%s] [Select=>%s]"),
...
UINT MyMessageBox(UINT type, TCHAR* title,  TCHAR* YesBut, TCHAR* NoBut, TCHAR* message ...)
{
	return 0;
//	CMyMessageBoxDlg dlg;
	CMsgDlg dlg;
...
//	strTmp2.Format(_T("%s"), cTmp);

	uiTmp = GetPrivateProfileString(_T("LANGUAGE"), _T("100"), _T("language"), language_select, 200, _T(".\\Data\\DisMessage.dat"));
	if( _ttoi(language_select) == 1) swprintf_s(MessageBuf, _T("%s"),cTmp);
	else if( _ttoi(language_select) == 2) swprintf_s(MessageBuf,_T("%s"),cTmp2);
	else if( _ttoi(language_select) == 3) swprintf_s(MessageBuf,_T("%s\n\n%s"),cTmp2, cTmp);
	else swprintf_s(MessageBuf,_T("언어선택 오류!!!"));

	return(MyMessageBox(MessageBuf,title,type,YesBut,NoBut));
}

UINT MyMessageBox(TCHAR* tmp, UINT i, TCHAR* title, UINT type, TCHAR* YesBut/*=_T("예")*/, TCHAR* NoBut/*=_T("아니오")*/)
{
	CString strTmp,aaa;
	TCHAR eMsg[200];
	TCHAR cTmp[200];
...
//	UINT uiTmp = GetPrivateProfileString("MESSAGE", strTmp, "error", cTmp, 200, ".\\Data\\DisMessage.dat");
	...
	aaa.Format(_T("%s"), tmp);
	strTmp.Format(_T("%s"), cTmp);
	swprintf_s(eMsg,strTmp,tmp);
	strTmp = aaa+strTmp;
	//AfxMessageBox(strTmp);
	return(MyMessageBox(eMsg,title,type,YesBut,NoBut));
}

BOOL GetNumberBox(TCHAR* szNum, int maxchar, TCHAR* getnum, TCHAR* title, BOOL pwd/*=FALSE*/)
{
 /*  //CGetNum dlg(maxchar, getnum, title, NULL, pwd);
   CGetNumDlg dlg(maxchar, getnum, title, NULL, pwd);
   if( dlg.DoModal() == IDOK )
   {
	  l_tcscpy(szNum, dlg.m_strGetNum);
	  return TRUE;
   }
   else */
	return FALSE;
...
BOOL GetValueBox(TCHAR* szVal, int maxchar, TCHAR* getval, TCHAR* title, BOOL pwd/*=FALSE*/, BOOL btndisable/*= FALSE*/ )
{
 /*  CGetValueDlg dlg(maxchar, getval, title, NULL, pwd, btndisable);

   if( dlg.DoModal() == IDOK )
   {
      l_tcscpy(szVal, dlg.m_strGetVal);
      return TRUE;
   }
   else */
   return FALSE;
...
DWORD HexToInt(TCHAR* s)
{
	TCHAR hexch[] = _T("0123456789ABCDEF");
	int i,j;
	DWORD r,n,k;
	TCHAR ch;

	k=1; r=0;
	for(i=_tcslen(s);  i>0; i--) 
	{
...
BOOL ErrorDisplay(UINT err)
{
	...
	TCHAR cTmp[500], cTmp2[200], MessageBuf[200];
	...
	//////////////////////////////////////////////
	if(( _wfopen_s( &fp, ErrorDataFileName, _T("rt") ) ) == NULL)
	{
	...
	//strTmp.Format(_T("%s"), cTmp);
	strTmp.Format(_T("%ld"),err+40000); //영문출력
	uiTmp = GetPrivateProfileString(_T("PCBERROR"), strTmp, _T("Cannot find the Error Number in the Error File."), cTmp2, 200, _T(".\\Data\\Error.dat"));

	//ckh1024
	uiTmp = GetPrivateProfileString(_T("LANGUAGE"), _T("100"), _T("language"), language_select, 200, _T(".\\Data\\Error.dat"));
	if( _ttoi(language_select) == 1) swprintf_s(MessageBuf,_T("%s"),cTmp);
	else if( _ttoi(language_select) == 2) swprintf_s(MessageBuf,_T("%s"),cTmp2);
	else if( _ttoi(language_select) == 3) swprintf_s(MessageBuf,_T("%s\n%s"),cTmp2, cTmp);
	else swprintf_s(MessageBuf, _T("language Select Error!!!"));
	//ckh1024
...
//	}
	...
	swprintf_s(cTmp,_T("%s,%s,%s"),strErrNo,MessageBuf,strTime);
	//CString ErrorSave;
	//ErrorSave.Format(_T("%s"),cTmp);
...
//	BOOL buzzerflag = FALSE;
	...
	if( _wfopen_s( &fp, ErrorDataFileName, _T("rt") ) ) 
	{
		MyMessageBox(17205, _T("Error"),M_ICONERROR);
					//17205=Error File을 Open할 수가 없습니다.
		return FALSE;
	...
	if( !get_fpoint(fp,tmp,buf, _countof(buf)))
	{
		_itot_s(99999,tmp,10);
		_tcscat_s(tmp, _T(":"));
		if( !get_fpoint(fp,tmp,buf, _countof(buf)))
		{
			fclose(fp);
			swprintf_s(buf, _T("Cannot find the Error Number in the Error File. : (%d) I am sorry, but can you call Mechatronics Center with Memo of this Number and Step!!!"), err);
			swprintf_s(buf, _T("%d"),err);
...
//	char fname[100] = ".\\data\\ErrorList.dat";
	TCHAR fname1[100] =  _T(".\\data\\ErrorList.bak");
	...
	*/
	if( _wfopen_s( &fsource, _T(".\\data\\ErrorList.dat"), _T("w+") ) )
	{
...
//	errno_t ferr;
	...
	else if( _wfopen_s( &ftarget, _T(".\\data\\ErrorList.bak"), _T("r") ) )
	{
...
BOOL FileDeleteAll(TCHAR * pn)
{
	...
	MyProgressWnd ProgWnd(NULL, _T("Call the Model....."));
	ProgWnd.GoModal() ; 
	...
	for(i=0;i<filecnt;i++)
	{
		swprintf_s(buf,_T("%s\\%s"), pn, (LPCTSTR)filename[i]);
		CFile::Remove(buf);
...
BOOL CopyFileDir(TCHAR* dest, TCHAR* src, int deletemode/*=FALSE*/)
{
	...
	swprintf_s(buf,_T("%s\\*.*"),src);
	cnt = FileCountInDir(buf);

	if( !cnt )
	{
		MyMessageBox(17213, _T("Confirm"),M_ICONSTOP);
	...
	MyProgressWnd ProgWnd(NULL, _T("Model Copy....."));
	ProgWnd.GoModal() ; 
	...
	for(i=0; i<cnt; i++)
	{
		swprintf_s(srcfile, _T("%s\\%s"), src, filename[i]);
		swprintf_s(destfile, _T("%s\\%s"), dest, filename[i]);
...
/*
	...
	_stprintf(str+tmplen, "%02X%c%c",sum&0xFF, 0x2A, 0x0D);
*/
}
...
/*
	...
	ctmp[3] = TCHAR(Temp/1000);
	itmp = int(Temp)%1000;
	ctmp[2] = TCHAR(itmp/100);
	itmp = int(Temp)%100;
	ctmp[1] = TCHAR(itmp/10);
	itmp = int(Temp)%10;
	ctmp[0] = TCHAR(itmp);

	sprintf_s(buf, "@%02dWS01%X%X%X%1d", UtNo, ctmp[3], ctmp[2], ctmp[1], ctmp[0]);	
	TempCheckSum(buf);
*/
...
//	_tcscpy_s((buf, gMainView->m_sCom2Buff);
