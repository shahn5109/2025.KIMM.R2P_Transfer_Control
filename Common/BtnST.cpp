...
/*
	if(m_bJogMode == TRUE && m_iJogRun == 0)
	{
		CRect rt;
		CPoint cp = point;
		CWnd*				wndMyActive = this;
		wndMyActive = GetActiveWindow();
		wndMyActive->GetWindowRect(&rt);
//		((CWnd*)GetActiveWindow())->GetWindowRect(&rt);
		cp -= rt.TopLeft();

		m_iSelectButton = -1;
		for(int i=0; i<BUTTON_NUMBER; i++)
			if(m_MouseRect[i].PtInRect(cp))
			{
//				TRACE("i=%d, cp.x=%d, cp.y=%d\n", i, cp.x, cp.y);
				m_iSelectButton = i;
				break;
			}
	}
